##########################################################################################
# R script for processing and analyzing Taplin IP-MS data using SAINTexpress-int
...
#     Interaction file: sample name, bait name, accession_number, sum intensity
#     (skip entries with NA intensity)
# (5) run SAINTexpress-int via command line
...
output_filter <- fs::path(baitDir,paste0(ipName,'.SAINTexpressInt.FilteredProteins.txt'))
...
output_bait <- fs::path(baitDir,paste0(ipName,'.SAINTexpressInt.Bait'))
write.table(baitDf,output_bait,quote=F,sep='\t',row.names=F,col.names=F)

output_prey <- fs::path(baitDir,paste0(ipName,'.SAINTexpressInt.Prey'))
...
for (samp in scCols) {
	tempDf <- mutate(outDf,sample=samp,bait=baitDf$Bait[baitDf$Sample==samp]) %>%
		...
		na.omit %>% rename(intensity={{samp}})

	interDf <- rbind(interDf,tempDf)
}

output_int <- fs::path(baitDir,paste0(ipName,'.SAINTexpressInt.Int'))
...
system(str_glue(
	'cd {baitDir} && ',
	'SAINTexpress-int ',
	'{ipName}.SAINTexpressInt.Int ',
	'{ipName}.SAINTexpressInt.Prey ',
	'{ipName}.SAINTexpressInt.Bait ',
	'&& mv list.txt {ipName}.SAINTexpressInt.Stats.txt'
))

cat('*** ran SAINTexpress-int\n')

# ---------------------------------------------------------------------------------------
# (6) OUTPUT reformatted results table (with InWeb annotation)

# read in the saintexpress stats file
output_stats <- fs::path(baitDir,paste0(ipName,'.SAINTexpressInt.Stats.txt'))
SaintStatsInt <- read.delim(output_stats) %>% rename(gene = "PreyGene")

# replace zero BFDR with 1/2 of next smallest BFDR to avoid plotting -log10(0)
min_nonzero <- min(SaintStatsInt$BFDR[SaintStatsInt$BFDR > 0]) # minimum non-zero BFDR

SaintStatsInt$BFDR_plot <- ifelse(
	SaintStatsInt$BFDR == 0, # if BFDR is zero:
	min_nonzero / 2, # BFDR_plot == 1/2 of min(BFDR)
	SaintStatsInt$BFDR # else: BFDR_plot == BFDR 
)

# make a logFC column that takes log2 of the FoldChange column
SaintStatsInt$logFC <- log2(SaintStatsInt$FoldChange)

# make a significance column
SaintStatsInt$significant <- SaintStatsInt$BFDR <= 0.1
...
if (is.null(inwebDf)) {
	...
	inwebOverlap <- calc_hyper(SaintStatsInt,inwebDf,
		data.frame(listName='InWeb',intersectN=T),bait=bait)
}

SaintStatsInt$InWeb <- SaintStatsInt$gene %in% inwebInts

# output results table 
write.table(SaintStatsInt,output_stats,quote=F,sep='\t',row.names=F)

cat('*** output SAINTexpress-int stats table:',
	paste0(ipName,'.SAINTexpressInt.Stats.txt\n'))

# ---------------------------------------------------------------------------------------
# (7) OUTPUT volcano plots (color: significance, InWeb overlay)

output_volcanos <- fs::path(baitDir,paste0(ipName,'.SAINTexpressInt.VolcanoPlots.pdf'))
pdf(output_volcanos,height=4,width=4)

p <- ggplot(SaintStatsInt,aes(x=logFC,y=-log10(BFDR_plot))) +
...
if (!is.null(inwebOverlap)) {
  ...
  p <- ggplot(SaintStatsInt,aes(x=logFC,y=-log10(BFDR_plot))) +
    ...
    geom_text_repel(subset(SaintStatsInt, gene==bait | InWeb), mapping=aes(label=gene),
                    arrow=arrow(length=unit(0.015, 'npc')), box.padding=unit(0.1, "lines"),
                    point.padding=unit(0.15, "lines"), color="black", size=2) +
...
cat('*** output volcano plots:',paste0(ipName,'.SAINTexpressInt.VolcanoPlots.pdf\n'))
cat('*** SAINTEXPRESS-INT SCRIPT COMPLETED\n')
